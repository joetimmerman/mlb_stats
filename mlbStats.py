...
#parseInnings('http://gd2.mlb.com/components/game/mlb/year_2016/month_04/day_23/gid_2016_04_23_oakmlb_tormlb_1/inning/inning_all.xml')
recentDays()
#runErrors()
...
parseInnings('http://gd2.mlb.com/components/game/mlb/year_2014/month_05/day_12/gid_2014_05_12_miamlb_lanmlb_1/inning/inning_all.xml')
        ...
        for element in child.iter('td'):
            if element.text == None:
                for el in element.iter():
                    if el.tag == 'a':
                        zTemp.append(el.text)
            else:
                zTemp.append(element.text)
        if len(zTemp) == 8:
            zTemp.pop(0)
        zTemp.insert(0,i)
        i+=1
        zout.append(zTemp)
insertRecords('park_factors',zout,getColumns('park_factors'))


# In[15]:

def winReg(i, streak):
    if i == len(team_win_lose)-1:
        return(streak)
    elif team_win_lose[i]['game_id'][:4] != team_win_lose[i+1]['game_id'][:4]:
        return(streak)
    elif team_win_lose[i]['team_id'] != team_win_lose[i+1]['team_id']:
        return(streak)
    elif team_win_lose[i]['win'] != team_win_lose[i+1]['win']:
        return(streak)
    else:
        if team_win_lose[i]['win'] == 0:
            return(winReg(i+1,streak-1))
        else:
            return(winReg(i+1,streak+1))
sql_dir = "C:\\Users\\evan.marcey\\Documents\\mlbStats\\sql\\"
team_win_lose_file = "team_win_lose.sql"
def getSQL(query_file):
    with open(sql_dir+query_file,'r') as qf:
        query = qf.read()
    conn.execute(query)
    query_result = conn.fetchall()
    return(query_result)
        
team_win_lose = getSQL(team_win_lose_file)
t1 = time.time()
tout = []
for i in range(0,len(team_win_lose)):
    ttemp = []
    if team_win_lose[i-1]['win'] == 0:        
        win_lose = -1
    else:
        win_lose = 1
    team_win_lose[i]['streak'] = winReg(i-1,win_lose)
    ttemp = [
        team_win_lose[i]['game_id'],
        team_win_lose[i]['team_id'],
        team_win_lose[i]['game_year'],
        team_win_lose[i]['game_date'],
        team_win_lose[i]['streak'],
        team_win_lose[i]['win'],
        team_win_lose[i]['home_away'],
        team_win_lose[i]['team_errors'],
        team_win_lose[i]['game_pk']
    ]
    tout.append(ttemp)
t2 = time.time()
print("%s seconds elapsed" % round(t2-t1,3))
#print(team_win_lose[0])
insertRecords('team_win_lose',tout,getColumns('team_win_lose'))
connection.commit()
connection.close()
