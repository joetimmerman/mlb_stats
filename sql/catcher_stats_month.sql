SELECT m1.catcher_id,
   ...
   FROM (SELECT bbx.playerID catcher_id,
          substring(ab.gameID,1,7) record_month,
          count(1) num_pitches,
          SUM(CASE
                  WHEN p.zone > 9
   ...
   GROUP BY bbx.playerID,
            substring(ab.gameID,1,7)) m1
   LEFT JOIN (SELECT bbx.playerID catcher_id,
          substring(a.gameID,1,7) record_month,
   ...
   GROUP BY bbx.playerID,
            substring(a.gameID,1,7)) wp_pb
ON wp_pb.catcher_id = m1.catcher_id
   AND wp_pb.record_month = m1.record_month;